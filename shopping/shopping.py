    ...
    with open('shopping.csv', mode='r') as file:
        ...
        for row in csv_reader:
            ...
            evidence.append([
                ...
                MONTHS[row[10]],  # Month (convert to int)
                int(row[11]),  # OperatingSystems
                int(row[12]),  # Browser
                int(row[13]),  # Region
                int(row[14]),  # TrafficType
                1 if row[15] == "Returning_Visitor" else 0,  # VisitorType (1 or 0)
                1 if row[16] == "TRUE" else 0  # Weekend (1 or 0)
            ])

            # Append label (1 for TRUE, 0 for FALSE)
    ...
    for i in range(len(predictions)):
        if predictions[i] == labels[i]:
            if predictions[i]:
                ...
                falses_predictions += 1
        if labels[i]:
            trues += 1
        else:
            falses += 1

    sensitivity = trues_predictions / trues
    specificity = falses_predictions / falses
